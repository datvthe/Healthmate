...
const CommunityFeed = () => {
    ...
    useEffect(() => {
        ...
        fetch("https://healthmate-y9vt.onrender.com/api/community/leaderboard")
            .then(res => res.json())
...
const ChallengesView = ({ user, setActiveView }: any) => {
    ...
    const handleJoin = async (id: string) => {
        if (!token) return;
        const res = await fetch(`https://healthmate-y9vt.onrender.com/api/community/challenges/${id}/join`, {
...
const PostCard = ({ post, currentUserId, token, navigate, onUpdate }: any) => {
    ...
    const toggleAction = async (action: 'like' | 'save') => {
        if (!token) return navigate("/login");
        try {
            const response = await fetch(`https://healthmate-y9vt.onrender.com/api/community/posts/${post._id}/${action}`, {
                method: "PUT",
                headers: { "Content-Type": "application/json", "Authorization": `Bearer ${token}` }
            });
            if (response.ok) {
    ...
    const submitComment = async () => {
        ...
        try {
            const response = await fetch(`https://healthmate-y9vt.onrender.com/api/community/posts/${post._id}/comment`, {
                method: "POST",
                headers: { "Content-Type": "application/json", "Authorization": `Bearer ${token}` },
